...
async function runAnalysis() {
  ...
  try {
    ...
    if (result) {
      ...
      state._ahaAnalyzing = false;
      state._ahaAnalysisStep = 'error';
      state._ahaErrorMsg = 'AI 서버가 응답하지 않았어요. 잠시 후 다시 시도해주세요.';
      navigate(state.currentScreen, { replace: true });
    }
  } catch (e) {
    ...
    state._ahaAnalyzing = false;
    state._ahaAnalysisStep = 'error';
    state._ahaErrorMsg = e.message?.includes('시간 초과')
    ...
    <div class="full-screen animate-slide">
      <div class="al-container">
        ${isError ? `
          ...
          <p style="color:var(--text-muted);font-size:13px;margin:12px 0 24px">${state._ahaErrorMsg || '네트워크 상태를 확인하고 다시 시도해주세요'}</p>
