/* ================================================================
   Records Module — views/volunteer-upload.js
   봉사활동 사진 업로드
   ================================================================ */
...
const PREFIX = '_vol';
const ACTIVITY_TYPE = 'volunteer';
const ACTIVITY_TITLE = '봉사활동';
const MAIN_TAG = '활동';
const BACK_SCREEN = 'record-activity';

export function registerHandlers(RM) {
  RM.volHandleMain = (input) => handleMainUpload(input);
  RM.volRemoveMain = () => removeMain();
  RM.volHandleRef = (input) => handleRefUpload(input);
  RM.volRemoveRef = (idx) => removeRefPhoto(idx);
  RM.volUpdateComment = (v) => { state[`${PREFIX}Comment`] = v; };
  RM.volSave = () => saveActivity(RM);
}
...
async function saveActivity(RM) {
  ...
  state._activityAnalysisStep = '봉사활동을 분석하고 있어요...';
  ...
  if (aiResult) {
    ...
    if (logId && RM.toast) RM.toast('봉사활동이 저장되었습니다 (AI 분석 실패)');
...
export function renderVolunteerUpload() {
  ...
  return `
    <div class="full-screen animate-slide">
      <div class="screen-header">
        ...
        <h1>봉사활동 기록</h1>
        <span class="header-badge" style="background:#06b6d4;color:#fff">봉사</span>
      </div>
      <div class="form-body">
        <div class="subject-indicator">
          <span class="subject-dot" style="background:#06b6d4"></span>
          <span>봉사활동 기록</span>
        </div>
        ...
        <div class="pu-note-section">
          <div class="pu-note-header">
            ...
            <span class="pu-note-subtitle">봉사활동</span>
          </div>
          <div class="pu-note-desc">봉사활동 현장을 촬영해주세요</div>
            ...
            <div class="pu-note-preview">
              ...
              <div class="pu-note-actions">
                <div class="pu-note-retake" style="cursor:pointer" onclick="this.querySelector('input').click()">
                  다시 촬영
                  <input type="file" accept="image/*"style="position:absolute;width:1px;height:1px;opacity:0;overflow:hidden" onchange="_RM.volHandleMain(this)">
                </div>
        ...
        <div class="pu-ref-section">
          <div class="pu-ref-header">
            <span class="pu-ref-title">📷 확인 자료</span>
            <span class="pu-ref-optional">(선택)</span>
          </div>
          <div class="pu-ref-desc">봉사확인서, 활동 안내문 등 확인 자료를 남겨보세요</div>

          <div class="pu-ref-grid">
            ${refPhotos.map((p, i) => `
              <div class="pu-ref-tile">
                <img class="pu-ref-tile-img" src="${p.dataUrl}" alt="참고 ${i + 1}">
                <button class="pu-ref-tile-delete" onclick="_RM.volRemoveRef(${i})">&times;</button>
              </div>
            `).join('')}
            ${refCount < 14 ? `
              <div class="pu-ref-add-tile" style="position:relative;cursor:pointer" onclick="this.querySelector('input').click()">
                ...
                <input type="file" accept="image/*" multiple style="position:absolute;width:1px;height:1px;opacity:0;overflow:hidden" onchange="_RM.volHandleRef(this)">
              </div>
            ` : ''}
          </div>
        ...
        <div class="pu-comment-section">
          ...
          <div class="pu-comment-desc">봉사활동 후 느낀 점이나 배운 점을 적어주세요</div>
          <textarea class="pu-comment-textarea"
                    placeholder="예) 봉사활동을 통해 나눔의 가치를 배웠어요"
                    rows="3"
                    oninput="_RM.volUpdateComment(this.value)">${state._volComment || ''}</textarea>
        </div>

        <div class="pu-actions">
          <button class="btn-primary pu-ai-btn ${hasAny ? '' : 'disabled'}"
                  ${hasAny ? '' : 'disabled'}
                  onclick="${hasAny ? '_RM.volSave()' : ''}">
