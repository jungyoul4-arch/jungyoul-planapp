/* ================================================================
   Records Module — views/club-upload.js
   동아리 활동 사진 업로드
   ================================================================ */
...
const PREFIX = '_club';
const ACTIVITY_TYPE = 'club';
const ACTIVITY_TITLE = '동아리 활동';
const MAIN_TAG = '활동';
const BACK_SCREEN = 'record-activity';

export function registerHandlers(RM) {
  RM.clubHandleMain = (input) => handleMainUpload(input);
  RM.clubRemoveMain = () => removeMain();
  RM.clubHandleRef = (input) => handleRefUpload(input);
  RM.clubRemoveRef = (idx) => removeRefPhoto(idx);
  RM.clubUpdateComment = (v) => { state[`${PREFIX}Comment`] = v; };
  RM.clubSave = () => saveActivity(RM);
}
...
async function saveActivity(RM) {
  ...
  state._activityAnalysisStep = '활동 기록을 분석하고 있어요...';
  ...
  if (aiResult) {
    ...
    if (logId && RM.toast) RM.toast('동아리 활동이 저장되었습니다 (AI 분석 실패)');
...
export function renderClubUpload() {
  ...
  return `
    <div class="full-screen animate-slide">
      <div class="screen-header">
        ...
        <h1>동아리 활동 기록</h1>
        <span class="header-badge" style="background:#6366f1;color:#fff">동아리</span>
      </div>
      <div class="form-body">
        <div class="subject-indicator">
          <span class="subject-dot" style="background:#6366f1"></span>
          <span>동아리 활동 사진 기록</span>
        </div>
        ...
        <div class="pu-note-section">
          <div class="pu-note-header">
            ...
            <span class="pu-note-subtitle">동아리 활동</span>
          </div>
          <div class="pu-note-desc">동아리 활동 현장을 촬영해주세요</div>
            ...
            <div class="pu-note-preview">
              ...
              <div class="pu-note-actions">
                <div class="pu-note-retake" style="cursor:pointer" onclick="this.querySelector('input').click()">
                  다시 촬영
                  <input type="file" accept="image/*"style="position:absolute;width:1px;height:1px;opacity:0;overflow:hidden" onchange="_RM.clubHandleMain(this)">
                </div>
        ...
        <div class="pu-ref-section">
          <div class="pu-ref-header">
            <span class="pu-ref-title">📷 활동 자료</span>
            <span class="pu-ref-optional">(선택)</span>
          </div>
          <div class="pu-ref-desc">포스터, 기획안, 활동지 등 참고 자료를 남겨보세요</div>

          <div class="pu-ref-grid">
            ${refPhotos.map((p, i) => `
              <div class="pu-ref-tile">
                <img class="pu-ref-tile-img" src="${p.dataUrl}" alt="참고 ${i + 1}">
                <button class="pu-ref-tile-delete" onclick="_RM.clubRemoveRef(${i})">&times;</button>
              </div>
            `).join('')}
            ${refCount < 14 ? `
              <div class="pu-ref-add-tile" style="position:relative;cursor:pointer" onclick="this.querySelector('input').click()">
                ...
                <input type="file" accept="image/*" multiple style="position:absolute;width:1px;height:1px;opacity:0;overflow:hidden" onchange="_RM.clubHandleRef(this)">
              </div>
            ` : ''}
          </div>
        ...
        <div class="pu-comment-section">
          ...
          <textarea class="pu-comment-textarea"
                    placeholder="예) 오늘 동아리에서 새로운 프로젝트를 시작했어요"
                    rows="3"
                    oninput="_RM.clubUpdateComment(this.value)">${state._clubComment || ''}</textarea>
        </div>
